...
const DataFetchServer = async ({ searchParams }) => {
  ...
  let userData = { name: userName, gender: null, probability: 0 };

  if (userName) {
    const lowerName = userName.toLowerCase();
    const lookup = genderData[lowerName];
    console.log(`Local lookup for ${userName}:`, lookup);

    if (lookup && lookup.gender) {
      userData.gender = lookup.gender;
      userData.probability = lookup.probability || 0;
    }
  }
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          Server-side Analysis Engine v1.0
        </div>
      </div>
    </div>
  )
